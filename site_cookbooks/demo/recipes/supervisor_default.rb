...
template node['supervisor']['conffile'] do
  source "default_supervisor/supervisord.conf.erb"
  owner "root"
  group "root"
  mode "644"
  variables({
...
template "/etc/default/supervisor" do
  source "default_supervisor/debian/supervisor.default.erb"
  owner "root"
  group "root"
  mode "644"
...
when "amazon", "centos", "debian", "fedora", "redhat", "ubuntu", "raspbian"
  template "/etc/init.d/supervisor" do
    source "default_supervisor/#{init_template_dir}/supervisor.init.erb"
    owner "root"
    group "root"
    mode "755"
    variables({
...
when "smartos"
  ...
  template "/opt/local/share/smf/supervisord/manifest.xml" do
    source "default_supervisor/manifest.xml.erb"
    owner "root"
    group "root"
    mode "644"
