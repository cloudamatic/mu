...
# up in MU::MommaCat's ssh config wangling
			...
			def initialSSHTasks(ssh)
				...
				unless @config['use_cloud_provider_windows_password']
				...
				begin
					...
					if windows?
						...
						if !@config['hostname_set'] && @config['mu_windows_name']
							...
							raise BootstrapTempFail, "Setting hostname to #{@config['mu_windows_name']}, possibly rebooting"
						end
					end
...
#				end
				...
				if !@config['create_image'].nil?#and !chef_rerun_only
					...
					if img_cfg['image_then_destroy']
						...
						session.exec!("rm -rf /root/.ssh/authorized_keys /etc/ssh/ssh_host_*key* ; sed -i 's/^HOSTNAME=.*//' /etc/sysconfig/network")
						session.close
					end
					...
					if img_cfg['image_then_destroy']
						...
						@groomer.groomer_class.cleanup(node, @config['vault_access'])
						return
					end
				end
			...
			def self.createImage(name: name,
				...
				MU::MommaCat.createStandardTags(resp.image_id, region: region)
			  MU::MommaCat.createTag(resp.image_id, "Name", name, region: region)
				MU.log "AMI of #{name} in region #{region}: #{resp.image_id}"
				if !copy_to_regions.nil? and copy_to_regions.size > 0
					MU::Cloud::AWS::Server.waitForAMI(resp.image_id, region: region)
					copy_to_regions.each { |r|
						copy = MU::Cloud::AWS.ec2(r).copy_image(
							source_region: region,
							source_image_id: resp.image_id,
							name: name,
							description: "Image automatically generated by Mu from #{name}"
						)
