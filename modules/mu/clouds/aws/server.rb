...
#				end
				...
				if @config['create_ami'] and !chef_rerun_only
					if @config['image_then_destroy']
						# tear out Chef or Puppet or what have you
						@groomer.preClean

						# Scrub other things that don't belong on an AMI
						session = getSSHSession
						session.exec!("rm -rf /root/.ssh/authorized_keys; sed -i 's/^HOSTNAME=.*//' /etc/sysconfig/network")
						session.close
					end
					...
					if @config['image_then_destroy']
						...
						@groomer.cleanup(node, @config['vault_access'])
						return
					end
				end
