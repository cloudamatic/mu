...
# up in MU::MommaCat's ssh config wangling
			...
			def self.initialSSHTasks(ssh, server)
				...
				unless server['use_cloud_provider_windows_password']
					...
					if @config['use_cloud_provider_windows_password']
						...
						if @config["windows_auth_vault"].has_key?("password_field")
							win_admin_password = ChefVault::Item.load(
								@config['windows_auth_vault']['vault'],
						...
						if @config["windows_auth_vault"].has_key?("ec2config_password_field")
							ec2config_password = ChefVault::Item.load(
								@config['windows_auth_vault']['vault'],
								@config['windows_auth_vault']['item']
							)[@config["windows_auth_vault"]["ec2config_password_field"]]
						end
						...
						if @config["windows_auth_vault"].has_key?("sshd_password_field")
							sshd_password = ChefVault::Item.load(
								@config['windows_auth_vault']['vault'],
								@config['windows_auth_vault']['item']
							)[@config["windows_auth_vault"]["sshd_password_field"]]
						end
					end
