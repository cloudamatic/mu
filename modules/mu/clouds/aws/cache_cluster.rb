...
module MU
      ...
      class CacheCluster < MU::Cloud::CacheCluster
        ...
        def create
          ...
          if @config["create_replication_group"]
            ...
            resp = MU::Cloud::AWS.elasticache(region: @config['region']).create_replication_group(config_struct).replication_group

            wait_start_time = Time.now
            retries = 0
            begin
              MU::Cloud::AWS.elasticache(region: @config['region']).wait_until(:replication_group_available, replication_group_id: @config['identifier']) do |waiter|
            ...
            resp = MU::Cloud::AWS.elasticache(region: @config['region']).create_cache_cluster(config_struct).cache_cluster

            wait_start_time = Time.now
            retries = 0
            begin
              MU::Cloud::AWS.elasticache(region: @config['region']).wait_until(:cache_cluster_available, cache_cluster_id: @config['identifier']) do |waiter|
                waiter.max_attempts = nil
                waiter.before_attempt do |attempts|
        ...
        def createSubnetGroup
          ...
          if subnet_ids.empty?
            ...
            resp = MU::Cloud::AWS.elasticache(region: @config['region']).create_cache_subnet_group(
              cache_subnet_group_name: @config["subnet_group_name"],
  ...
  # XXX if a user doesn't set up dependencies correctly, this can die horribly on a NAT that's still in mid-creation. Fix this... possibly in the config parser.
        ...
        def createParameterGroup        
          ...
          resp = MU::Cloud::AWS.elasticache(region: @config['region']).create_cache_parameter_group(
          ...
          if @config.has_key?("parameter_group_parameters") && !@config["parameter_group_parameters"].empty?
            ...
            MU::Cloud::AWS.elasticache(region: @config['region']).modify_cache_parameter_group(
              cache_parameter_group_name: @config["parameter_group_name"],
        ...
        def getParameterGroup
          MU::Cloud::AWS.elasticache(region: @config['region']).describe_cache_parameter_groups(
            cache_parameter_group_name: @config["parameter_group_name"]
        ...
        def self.getCacheReplicationGroupById(repl_group_id, region: MU.curRegion)
          MU::Cloud::AWS.elasticache(region: region).describe_replication_groups(replication_group_id: repl_group_id).replication_groups.first
        end
        ...
        def createNewSnapshot
          ...
          attempts = 0
          begin
           snapshot = MU::Cloud::AWS.elasticache(region: @config['region']).create_snapshot(
          ...
          loop do
            ...
            snapshot_resp = MU::Cloud::AWS.elasticache(region: @config['region']).describe_snapshots(snapshot_name: snap_id)
            attempts += 1
        ...
        def getExistingSnapshot
          MU::Cloud::AWS.elasticache(region: @config['region']).describe_snapshots(snapshot_name: @config["identifier"]).snapshots.first.snapshot_name
        ...
        def self.cleanup(skipsnapshots: false, noop: false, ignoremaster: false, region: MU.curRegion, flags: {})        
          all_clusters = MU::Cloud::AWS.elasticache(region: region).describe_cache_clusters
          our_clusters = []
          ...
          if deploy.original_config && deploy.original_config.has_key?("cache_clusters") && !deploy.original_config["cache_clusters"].empty?
            ...
            all_clusters.cache_clusters.each { |cluster|
              ...
              begin
                tags = MU::Cloud::AWS.elasticache(region: region).list_tags_for_resource(resource_name: arn).tag_list
          ...
          if %w{deleting deleted}.include?(cluster.cache_cluster_status)
            ...
            unless noop
              def self.clusterSkipSnap(cluster_id, region)
                ...
                MU::Cloud::AWS.elasticache(region: region).delete_cache_cluster(cache_cluster_id: cluster_id)
              end

              def self.clusterCreateSnap(cluster_id, region)
                MU.log "Terminating #{cluster_id}. Final snapshot name: #{cluster_id}-mufinal"
                MU::Cloud::AWS.elasticache(region: region).delete_cache_cluster(cache_cluster_id: cluster_id, final_snapshot_identifier: "#{cluster_id}-MUfinal")
              end
              ...
              retries = 0
              begin
                MU::Cloud::AWS.elasticache(region: region).wait_until(:cache_cluster_deleted, cache_cluster_id: cluster_id) do |waiter|
                  waiter.max_attempts = nil
                  waiter.before_attempt do |attempts|
        ...
        def self.delete_subnet_group(subnet_group_id, region: MU.curRegion)
          ...
          MU::Cloud::AWS.elasticache(region: region).delete_cache_subnet_group(cache_subnet_group_name: subnet_group_id)
        ...
        def self.delete_parameter_group(parameter_group_id, region: MU.curRegion)
          ...
          MU::Cloud::AWS.elasticache(region: region).delete_cache_parameter_group(
