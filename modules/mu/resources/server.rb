      ...
      return [nat_ssh_key, nat_ssh_user, nat_ssh_host]
		...
		def self.initialSSHTasks(ssh, server)
			...
			if !server['use_cloud_provider_windows_password']
				# This covers both the case where we have a windows password passed from a vault and where we need to use a a random Windows Admin password generated by MU::Server.generateWindowsPassword
		...
		def self.groomEc2(server, instance, keypairname, environment: environment, sync_wait: sync_wait)
		  ...
		  MU::MommaCat.createTag(instance.instance_id, "Name", node, region: server['region'])
			...
			if %w{win2k12r2 win2k12 windows}.include?(server['platform'])
				...
				win_admin_password =  MU::Server.generateWindowsPassword if win_admin_password.nil?
				ec2config_password = MU::Server.generateWindowsPassword if ec2config_password.nil?
				sshd_password = MU::Server.generateWindowsPassword if sshd_password.nil?

				# We're creating the vault here so when we run MU::Server.initialSSHTasks and we need to set the Windows Admin password we can grab it fro a vault.
				win_password = "\"password\":\"#{win_admin_password}\", \"username\":\"#{server['windows_admin_username']}\", "
