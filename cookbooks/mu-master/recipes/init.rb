...
when 'rhel'
  ...
  when 7
    basepackages.concat(['libX11', 'mariadb-devel', 'cryptsetup', 'tcl-devel', 'gdbm-devel', 'sqlite-devel', 'tk-devel', 'perl-CGI', 'perl-DBI', 'perl-Data-Dumper', 'perl-Digest-MD5', 'perl-Git-SVN', 'perl-YAML', 'nvme-cli'])
...
rubies.each { |rubydir|
  ...
  execute "#{bundler_path} install from #{gemfile_dir}" do
    ...
    cwd gemfile_dir
    umask 0022
    not_if { !RUNNING_STANDALONE and system("cd #{gemfile_dir} && #{bundler_path} check"); $?.exitstatus }
    notifies :run, "bash[fix #{rubydir} gem permissions]", :delayed
